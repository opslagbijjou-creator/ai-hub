    ...
    <header className="fixed top-0 w-full z-50 bg-[#f7f9fb]/60 backdrop-blur-xl shadow-sm shadow-indigo-500/5">
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        <button className="flex items-center gap-3" onClick={() => navigate('/')}>
          <img src="/favicon.svg" alt="Belliq logo" className="w-8 h-8" />
          <div className="text-2xl font-bold tracking-tighter text-indigo-700">Belliq</div>
        </button>
        ...
        <div className="flex items-center gap-4">
          <button className="material-symbols-outlined text-slate-600 hover:text-indigo-600 transition-colors">language</button>
          <button
      ...
      <div className="md:hidden max-w-7xl mx-auto px-6 pb-3 flex items-center gap-3 overflow-x-auto">
        {links.map((link) => (
          <button
            key={link.key}
            onClick={() => navigate(link.path)}
